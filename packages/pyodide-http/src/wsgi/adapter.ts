...
export class WSGIAdapter {
  ...
  private options: Required<WSGIAdapterOptions>;
  private config: Required<Omit<AdapterConfig, 'cookieStorage'>> & { cookieStorage?: Storage };
  private cookieJar: CookieJar;
  private wsgiAppInitialized: boolean = false;
   ...
   */
  private async buildEnviron(request: BrowserRequest): Promise<Record<string, any>> {
    ...
    // Build base environ
    const environ: Record<string, any> = {
  ...
  async handleRequest(
    ...
    wsgiAppPath: string = 'application'
...
output
   ...
   */
  private buildWSGIAppImport(wsgiAppPath: string): string {
