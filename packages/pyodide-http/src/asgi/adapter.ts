...
import { headersToASGI, parseASGIHeaders, parseWSGIHeaders } from '../utils/headers.js';
...
export class ASGIAdapter {
  ...
  private options: Required<ASGIAdapterOptions>;
  private config: Required<Omit<AdapterConfig, 'cookieStorage'>> & { cookieStorage?: Storage };
  ...
  constructor(
    ...
    this.cookieJar = new CookieJar(
      this.config.cookieStorageKey,
      this.config.persistCookies ? this.config.cookieStorage : undefined
    );
  }
   ...
   */
  private async buildScope(request: BrowserRequest): Promise<Record<string, any>> {
  ...
  async handleRequest(
    ...
    asgiAppPath: string = 'application'
...
output
      ...
      const response: BrowserResponse = {
        ...
        headers,
        cookies: cookies.map(([_name, value]: [string, string]) => {
   ...
   */
  private buildASGIAppImport(asgiAppPath: string): string {
